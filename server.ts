...
app.use(['/', '/pokemonss'], async (req: CustomRequest, res, next) => {
...
app.use(
  ...
  async (req: CustomRequest, res, next) => {
    ...
    try {
      const { data } = await fetchPokemon(req.pokemonResult.name);
...
app.get('*', async (req: CustomRequest, res) => {
  ...
  const ReactComponent = await renderOnServer(req.pokemonResult.image_url, props);
